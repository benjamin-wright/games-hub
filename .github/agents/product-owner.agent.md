...
3. **Check for conflicts and duplication.** Identify any existing spec language that contradicts, overlaps, or already describes the proposed feature. Flag these explicitly before proposing changes.
4. **Propose the minimum change.** Recommend only what is needed to represent the feature — no rewrites, no added context beyond what is necessary.
5. **Present the proposed change to the user for approval before editing any files.**
6. Once approved, apply the changes and use the **"Create Task"** handoff button to hand off to the scrum-master.
...
- **No project-wide standards** — specs must not restate information already covered by `docs/standards/`. Language choices, code-organisation conventions, testing strategies, and reusable tooling patterns belong in standards documents. If a spec line would be equally true of any component in the project, it does not belong in the spec.
- **No implementation detail** — describe observable behaviour and interfaces, not internal structure. How a feature is built (package layout, naming schemes for Kubernetes objects, env-var conventions) is implementation, not specification.
